// =========================
// Result + SubsidyDetail + List screens
// =========================

// ====== USR-04: Diagnose Result ======
const Result = ({ onOpenDetail, onSwitchMode, searchMode, isMobile }) => {
        ...
        <div style={{
            ...
            <aside>
              <div style={{ position: "sticky", top: 18 }}>
                ...
                <div className="col" style={{ gap: 0, marginBottom: 24 }}>
                  {[
                    { id: "all", l: "すべて", n: SUBSIDIES.length },
                    { id: "open", l: "公募中", n: SUBSIDIES.filter(s => s.status === "open").length },
                    { id: "pre", l: "公募前", n: SUBSIDIES.filter(s => s.status === "pre").length },
                    { id: "closed", l: "終了", n: 0 },
                  ].map(f => (
          ...
          <div>
            ...
            <div style={{ borderBottom: "1px solid var(--line)" }}>
              {items.map((s, i) => (
                <SubsidyCard key={s.id} s={s} onClick={() => onOpenDetail(s.id)} delay={i * 30} />
              ))}
            </div>
  ...
  <div className="fade-in" style={{
    ...
    <div style={{ display: "grid", gridTemplateColumns: "76px 1fr 220px auto", gap: 20, alignItems: "start" }}>
      ...
      <div className="col" style={{ gap: 10, alignItems: "flex-end" }}>
        <button className="btn btn-ghost btn-sm" onClick={e => e.stopPropagation()} style={{ padding: "4px 8px" }}>
          <Ico name="bookmark" size={11} />保存
        </button>
...
const AIChatSearch = ({ onOpenDetail }) => {
...
const SubsidyDetail = ({ subsidyId, onBack, onConsult, isMobile }) => {
  const s = SUBSIDIES.find(x => x.id === subsidyId) || SUBSIDIES[0];
    ...
    <div style={{ background: "var(--bg)" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr auto", gap: 18, alignItems: "start" }}>
          ...
          <div className="row" style={{ gap: 8 }}>
            <button className="btn btn-ghost" onClick={() => setWatched(!watched)}>
              <Ico name={watched ? "bookmarkF" : "bookmark"} size={12} />
              {watched ? "保存済" : "ウォッチ"}
            </button>
      ...
      <div style={{
        ...
        {[
          ...
          { l: "補助上限", v: window.formatYen(s.maxAmount), u: "" },
        ...
        {tab === "reasons" && (
          <div style={{ maxWidth: 800 }}>
            ...
            <div style={{ borderTop: "1px solid var(--line-ink)" }}>
              {[
                { l: "業種一致度", v: 95, n: "対象業種「ソフトウェア業」に完全一致" },
                { l: "規模適合度", v: 90, n: "従業員12名 — 小規模事業者要件（20名以下）に該当" },
                { l: "事業適合度", v: 91, n: "DX推進・クラウド導入が補助対象事業に明示" },
              ].map((r, i) => (
                <div key={i} style={{ padding: "22px 0", borderBottom: "1px solid var(--line)", display: "grid", gridTemplateColumns: "1fr auto 280px", gap: 24, alignItems: "center" }}>
                  <div>
...
Object.assign(window, { Result, SubsidyDetail, AdoptionChart, AIChatSearch });
